...
public class Data
{
   ...
   public String getByteString(int k)
   {
      ...
      for (int i = 0; i < 8; i++)  print = ((b >> i)&1) + print;
      return print;
   }
   ...
   public static Data crossover(Data d1,Data d2,int x)
   {
      ...
      for (int i = 0; i < bytex; i++)  
    	  bytelist.add(d1.getByte(i));
      int b1 = 0;  
      for (int k = 0; k < bitx; k++)  
    	  b1 = b1 + (1 << (7 - k));
      int b2 = 0;  for (int k = bitx; k < 8; k++) 
    	  b2 = b2 + (1 << (7 - k));
   ...
   public String toString()
   {
	...
	public void replaceBits(byte fromData, int subSetIndex) 
   {
   		data.set(subSetIndex, fromData);
   }
}
